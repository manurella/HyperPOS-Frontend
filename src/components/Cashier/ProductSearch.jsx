      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 relative" ref={suggestionsRef}>
          <div className="relative flex-1">
            <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8C8A82]" />
            <input
            ...
            {suggestions.length > 0 && (
              <ul className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-[#E8E5DC] rounded-lg shadow-lg max-h-48 overflow-y-auto">
                {suggestions.map(p => (
                  <li key={p.id}
                    onClick={() => { setSelectedProductId(p.id.toString()); setSearchTerm(p.name); setBarcode(p.barcode || ""); setSuggestions([]); }}
                    className="flex items-center justify-between px-3 py-2.5 hover:bg-[#FAFAF7] cursor-pointer text-sm text-[#1A1915] transition-colors border-b border-[#E8E5DC] last:border-0">
                    <span className="font-medium">{p.name}</span>
                    <span className="text-xs text-[#8C8A82] ml-4">Stock: {p.stock}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <div className="relative sm:w-52">
            <Barcode size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8C8A82]" />
            <input
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            <div key={label} className={`rounded-lg px-3 py-2 border ${highlight ? "bg-[#E6F0E1] border-[rgba(79,122,63,0.2)]" : "bg-[#FAFAF7] border-[#E8E5DC]"}`}>
              <p className={`text-xs font-semibold uppercase tracking-wide mb-0.5 ${highlight ? "text-[#4F7A3F]" : "text-[#8C8A82]"}`}>{label}</p>
              <p className={`text-sm font-semibold ${highlight ? "text-[#4F7A3F]" : "text-[#1A1915]"}`}>{value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-end">
          <div className="flex-1">
            <label className="block text-xs font-semibold text-[#8C8A82] uppercase tracking-wide mb-1.5">Product</label>
            <select
          ...
          <div className="sm:w-28">
            <label className="block text-xs font-semibold text-[#8C8A82] uppercase tracking-wide mb-1.5">Quantity</label>
            <input
