    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8E5DC]">
          <h2 className="text-base font-semibold text-[#1A1915]">Sale Details</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-[#8C8A82] hover:text-[#5C5A54] hover:bg-[#EEECE5] transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto px-6 py-5 space-y-4">
          <div>
            <h3 className="text-xs font-bold text-[#8C8A82] uppercase tracking-wider mb-3 pb-1 border-b border-[#E8E5DC]">Invoice</h3>
            <div className="grid grid-cols-2 gap-3 text-sm">
              {[
                ...
                <div key={label}>
                  <p className="text-xs font-semibold text-[#8C8A82] uppercase tracking-wide mb-0.5">{label}</p>
                  <p className="text-[#5C5A54]">{val}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3 flex items-center justify-between">
            <span className="text-sm text-[#5C5A54] font-medium">Total</span>
            <span className="text-lg font-bold text-emerald-600">Rs {sale.invoice.total.toLocaleString()}</span>
          </div>

          <div>
            <h3 className="text-xs font-bold text-[#8C8A82] uppercase tracking-wider mb-3 pb-1 border-b border-[#E8E5DC]">
              Items ({sale.items.length})
            </h3>
            <div className="space-y-2">
              {sale.items.map(item => (
                <div key={item.id} className="bg-[#FAFAF7] rounded-lg px-4 py-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-[#5C5A54]">Product #{item.productId}</p>
                    <p className="text-xs text-[#8C8A82]">Qty: {item.quantity} · Unit: Rs {item.unitPrice?.toLocaleString()} · Disc: {item.discount}%</p>
                  </div>
        ...
        <div className="px-6 py-4 bg-[#FAFAF7] border-t border-[#E8E5DC] flex justify-end">
    ...
    <div className="bg-white border border-[#E8E5DC] rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[#D4D0C4]">{icon}</span>
        </div>
        {isLoading
          ? <div className="h-7 w-24 bg-[#EEECE5] animate-pulse rounded-md" />
          : <p className="text-2xl sm:text-3xl font-bold text-[#1A1915] tracking-tight">{value}</p>
        }
      </div>
    </div>
  );
...
function BaseScreenHome() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-[#1A1915]">Cashier Dashboard</h1>
        <p className="text-sm text-[#8C8A82] mt-0.5">Today's overview and quick access</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<TrendingUp size={18} />}  label="Today's Sales"   value={`Rs ${stats.todaySales.toLocaleString()}`} accentColor="bg-[#E6F0E1]0"    isLoading={isLoading} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {navCards.map((card, i) => (
            <Link key={i} to={card.path}
              className="bg-white border border-[#E8E5DC] rounded-xl shadow-sm p-5 hover:border-[#E8E5DC] hover:shadow-sm transition-all duration-200 group flex flex-col">
              <div className="w-10 h-10 rounded-lg bg-[#EEECE5] flex items-center justify-center text-[#8C8A82] mb-4 group-hover:bg-[#E6F0E1] group-hover:text-[#4F7A3F] transition-colors">
                {card.icon}
              </div>
              <h3 className="text-base font-semibold text-[#1A1915] mb-1">{card.title}</h3>
              <p className="text-sm text-[#8C8A82] flex-1">{card.desc}</p>
              <div className="flex items-center gap-1 text-[#8C8A82] group-hover:text-[#4F7A3F] text-sm font-medium mt-4 transition-colors">
                <span>Open</span>
                ...
                {isLoading ? (
                  Array(5).fill(0).map((_, i) => (
                    <tr key={i}>
                      {Array(6).fill(0).map((__, j) => (
                        <td key={j}><div className="h-4 bg-[#EEECE5] animate-pulse rounded" /></td>
                      ))}
                    </tr>
                  ))
                ) : salesData.length > 0 ? (
                  salesData.slice(0, 8).map(sale => (
                    <tr key={sale.invoice.id}>
                      <td className="font-medium text-[#8C8A82] text-xs">{sale.invoice.id}</td>
                      ...
                      <td className="text-center">
                        <button onClick={() => setSelectedSale(sale)}
                          className="p-1.5 rounded-lg text-[#8C8A82] hover:text-[#4F7A3F] hover:bg-[#E6F0E1] transition-colors">
                          <Eye size={16} />
                        </button>
                      </td>
                    </tr>
                  ...
                  <tr><td colSpan="6" className="py-10 text-center text-[#8C8A82]">No transactions found</td></tr>
                )}
              </tbody>
            </table>
          </div>
              ...
              <tbody>
                {isLoading ? (
                  Array(3).fill(0).map((_, i) => (
                    <tr key={i}>{Array(5).fill(0).map((__, j) => <td key={j}><div className="h-4 bg-[#EEECE5] animate-pulse rounded" /></td>)}</tr>
                  ))
                ) : productData.filter(p => (p.quantity || 0) < 10).length > 0 ? (
                  productData.filter(p => (p.quantity || 0) < 10).slice(0, 5).map(p => (
                    <tr key={p.id}>
                      <td className="text-xs text-[#8C8A82] font-medium">{p.id}</td>
                      <td className="font-medium text-[#1A1915]">{p.name}</td>
                  ...
                  <tr><td colSpan="5" className="py-10 text-center text-[#8C8A82]">All items are well stocked</td></tr>
                )}
              </tbody>
            </table>
          </div>
