...
function PurchasePage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-slate-800">Purchase Management</h1>
        <p className="text-sm text-slate-500 mt-0.5">{purchaseData.length} total purchases</p>
      </div>
            ...
            <table className="pos-table">
              ...
              <tbody>
                {filteredData.length > 0 ? filteredData.map(p => (
                  <tr key={p.grn.id}>
                    <td className="font-medium text-slate-500 text-xs">{p.grn.id}</td>
                    <td className="text-slate-700">{p.grn.supplierId}</td>
                    <td className="font-semibold text-emerald-600">Rs {p.grn.total.toLocaleString()}</td>
                    <td className="hidden md:table-cell text-slate-500">{p.items.length}</td>
                    <td className="hidden lg:table-cell text-slate-500">{fmt(p.grn.updatedAt)}</td>
                    <td className="text-center"><button onClick={() => setSelectedPurchase(p)} className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" aria-label="View"><Eye size={16} /></button></td>
                  </tr>
                )) : <tr><td colSpan="6" className="py-12 text-center text-slate-400">No purchases found</td></tr>}
              </tbody>
            </table>
          </div>
        )}
