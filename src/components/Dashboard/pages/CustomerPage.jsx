  ...
  return (
    <ModalShell title="Filter Customers" onClose={onClose} footer={<><button onClick={reset} className="pos-btn-secondary">Reset</button><button onClick={() => { onApply({ name, email, phone, address, status }); onClose(); }} className="pos-btn-primary">Apply</button></>}>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label className="block text-xs font-semibold text-[#5C5A54] uppercase tracking-wide mb-1.5">Email</label><input value={email} onChange={e => setEmail(e.target.value)} className="pos-input" placeholder="Search email" /></div>
        <div><label className="block text-xs font-semibold text-[#5C5A54] uppercase tracking-wide mb-1.5">Phone</label><input value={phone} onChange={e => setPhone(e.target.value)} className="pos-input" placeholder="Search phone" /></div>
...
function CustomerPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-sans text-[28px] font-bold text-[#1A1915] tracking-[-0.02em]">Customer Management</h1>
        <p className="text-[13px] text-[#8C8A82] mt-0.5">{customerData.length} total customers</p>
      </div>
            ...
            <table className="pos-table">
              ...
              <tbody>
                {filteredData.length > 0 ? filteredData.map(c => (
                  <tr key={c.id}>
                    <td className="font-medium text-[#5C5A54] text-xs">{c.id}</td>
                    <td className="font-semibold text-[#1A1915]">{c.name}</td>
                    <td className="hidden sm:table-cell text-[#5C5A54]">{c.email || "N/A"}</td>
                    <td className="hidden md:table-cell text-[#5C5A54]">{c.phone || "N/A"}</td>
                    ...
                    <td className="text-center"><button onClick={() => setSelectedCustomer(c)} className="p-2 rounded-[7px] text-[#8C8A82] hover:text-[#4F7A3F] hover:bg-[#E6F0E1] transition-colors" aria-label="View"><Eye size={16} /></button></td>
                  </tr>
                )) : <tr><td colSpan="6" className="py-12 text-center text-[#8C8A82]">No customers found</td></tr>}
              </tbody>
            </table>
          </div>
        )}
