  ...
  return (
    <ModalShell title="Customer Details" onClose={onClose} footer={<button onClick={onClose} className="pos-btn-primary">Close</button>}>
      ...
      <InfoSection title="Contact">
        <InfoRow label="Email" full>{customer.email || "N/A"}</InfoRow>
        <InfoRow label="Phone">{customer.phone || "N/A"}</InfoRow>
        <InfoRow label="Address" full>{customer.address || "N/A"}</InfoRow>
      </InfoSection>
  ...
  return (
    <ModalShell title="Filter Customers" onClose={onClose} footer={<><button onClick={reset} className="pos-btn-secondary">Reset</button><button onClick={() => { onApply({ name, email, phone, address, status }); onClose(); }} className="pos-btn-primary">Apply</button></>}>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label className="block text-xs font-semibold text-zinc-600 uppercase tracking-wide mb-1.5">Email</label><input value={email} onChange={e => setEmail(e.target.value)} className="pos-input" placeholder="Search email" /></div>
        <div><label className="block text-xs font-semibold text-zinc-600 uppercase tracking-wide mb-1.5">Phone</label><input value={phone} onChange={e => setPhone(e.target.value)} className="pos-input" placeholder="Search phone" /></div>
...
function CustomerPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900">Customer Management</h1>
        <p className="text-sm text-zinc-600 mt-0.5">{customerData.length} total customers</p>
      </div>
            ...
            <table className="pos-table">
              ...
              <tbody>
                {filteredData.length > 0 ? filteredData.map(c => (
                  <tr key={c.id}>
                    <td className="font-medium text-zinc-600 text-xs">{c.id}</td>
                    <td className="font-medium text-zinc-900">{c.name}</td>
                    <td className="hidden sm:table-cell text-zinc-600">{c.email || "N/A"}</td>
                    <td className="hidden md:table-cell text-zinc-600">{c.phone || "N/A"}</td>
                    ...
                    <td className="text-center"><button onClick={() => setSelectedCustomer(c)} className="p-1.5 rounded-lg text-zinc-500 hover:text-blue-600 hover:bg-zinc-50 transition-colors" aria-label="View"><Eye size={16} /></button></td>
                  </tr>
                )) : <tr><td colSpan="6" className="py-12 text-center text-zinc-500">No customers found</td></tr>}
              </tbody>
            </table>
          </div>
        )}
