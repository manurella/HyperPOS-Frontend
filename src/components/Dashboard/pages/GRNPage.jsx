...
function GRNPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900">Goods Received Notes</h1>
        <p className="text-sm text-zinc-600 mt-0.5">{grnData.length} total GRNs</p>
      </div>
            ...
            <table className="pos-table">
              ...
              <tbody>
                {filteredData.length > 0 ? filteredData.map(g => (
                  <tr key={g.id}>
                    <td className="font-medium text-zinc-600 text-xs">{g.id}</td>
                    <td className="text-zinc-700">{g.supplierId}</td>
                    <td className="font-semibold text-emerald-600">Rs {g.total.toLocaleString()}</td>
                    <td className="hidden md:table-cell text-zinc-600">{fmt(g.updatedAt)}</td>
                    <td className="text-center"><button onClick={() => setSelectedGRN(g)} className="p-1.5 rounded-lg text-zinc-500 hover:text-blue-600 hover:bg-zinc-50 transition-colors" aria-label="View"><Eye size={16} /></button></td>
                  </tr>
                )) : <tr><td colSpan="5" className="py-12 text-center text-zinc-500">No GRNs found</td></tr>}
              </tbody>
            </table>
          </div>
        )}
