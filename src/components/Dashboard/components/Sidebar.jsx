...
import {
  ...
  PlusCircle, ClipboardList, ArrowLeftRight, X,
} from "lucide-react";
    ...
    >
      ...
      <div className="flex items-center justify-between h-[4.5rem] px-5 flex-shrink-0 border-b border-white/[0.07]">
        <Link to="/dashboard" className="flex items-center gap-2.5">
          ...
          <span className="text-white font-semibold text-[1rem] tracking-tight">HyperPOS</span>
        </Link>
        <button
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 hyper-scrollbar">
        ...
        <div className="mb-1">
          ...
          >
            <span className={isActive(dashboardItem.path, dashboardItem.exact) ? "text-blue-400" : "text-zinc-600 group-hover:text-zinc-400"}>
              {dashboardItem.icon}
            </span>
            {dashboardItem.title}
          </Link>
